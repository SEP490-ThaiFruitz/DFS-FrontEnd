...
import { ProductVariant, Request } from './list-plan';
import { REQUEST_KEY } from '@/app/key/comm-key';

function UpdatePlan() {
    const { id } = useParams()
    const { data: request, refetch } = useFetch<ApiResponse<Request>>(`/Requests/${id}`, [REQUEST_KEY.REQUEST_DETAIL_MANAGE, id as string])
    ...
    const form = useForm<z.infer<typeof CreatePlanSafeTypes>>({
        ...
        defaultValues: {
            ...
            requestType: request?.value?.requestType ?? "Import",
            expectedDate: request?.value?.expectedDate ? new Date(request.value.expectedDate) : undefined,
            requestItems: []
        }
    });
    ...
    useEffect(() => {
        if (request?.value) {
            form.reset({
                ...
                requestType: request?.value?.requestType ?? "Import",
                expectedDate: request?.value?.expectedDate ? new Date(request.value.expectedDate) : undefined,
                requestItems: request?.value?.productVariants.map((productVariant: ProductVariant) => (
                    {
                        ...productVariant,
    ...
    const onSubmit = async (values: z.infer<typeof CreatePlanSafeTypes>) => {
        try {
            ...
            const res = await API.update(`/Requests/${id}`, {
                ...restValues,
                id: id,
    ...
    return (
        <FormValues form={form} onSubmit={onSubmit} classNameForm="m-10">
            <Card>
                <CardHeader>
                    <CardTitle>Cập nhật kế hoạch</CardTitle>
                </CardHeader>
                <CardContent>
