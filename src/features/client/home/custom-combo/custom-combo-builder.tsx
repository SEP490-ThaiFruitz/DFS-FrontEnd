...
export function CustomComboBuilder() {
  ...
  useEffect(() => {
    ...
    setProducts(productsData?.value.items);
  }, []);
  ...
  const filteredProducts = products?.filter((product) => {
    ...
    const matchesTags =
      ...
      activeTags.every((tag) =>
        product.tags.some(
          (productTag) => productTag.trim().toLowerCase() === tag.toLowerCase()
        )
      );
  ...
  const addToCombo = (product: Product, variant: ProductVariant) => {
    ...
    toast.success(`Đã thêm ${product.name} - ${variant.packageType} vào combo`);
  };
  ...
  const removeItem = (productVariantId: string) => {
    setSelectedItems(
      selectedItems.filter((item) => item.productVariantId !== productVariantId)
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    toast.success(
      `"${comboName}" với ${selectedItems.length} sản phẩm đã được tạo. `
    );
          ...
          <ComboDropZone>
            <div className="rounded-xl shadow-sm p-6 sticky top-4 cardStyle">
              ...
              <Button
                ...
                disabled={selectedItems.length === 0 || comboName.trim() === ""}
              >
                Tạo combo
              </Button>
            </div>
