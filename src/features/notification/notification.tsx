...
const Notification = () => {
    ...
    const { data: notifications, refetch, isLoading } = useFetch<ApiResponse<NotificationData>>(
        ...
        <Popover>
            <PopoverTrigger>
                ...
                rounded-full flex items-center justify-center"
                        >
                            {notifications?.value?.totalUnread > 9 ? '9+' : notifications?.value?.totalUnread}
                        </span>
                    )}
                </div>
            </PopoverTrigger>
                    ...
                    <div className="p-4 border-b">
                        ...
                        {notifications?.value && notifications?.value?.totalUnread > 0 && (
                            <p className="text-sm text-muted-foreground">
                                Bạn có {notifications.value.totalUnread} thông báo chưa đọc {notifications.value.totalUnread !== 1 ? 's' : ''}
                            </p>
                        )}
                    </div>
                    <Tabs defaultValue={activeTab} onValueChange={handleTabChange} className="w-full">
                        ...
                        <TabsContent value={activeTab} className="mt-0">
                            <ScrollArea className="max-h-[450px] overflow-y-auto">
                                <NotificationList
                                    notifications={notifications?.value?.notifications?.items ?? []}
                                    isLoading={isLoading}
                                    onLoadMore={handleLoadMore}
                                    hasMore={notifications?.value?.notifications?.hasNextPage || false}
                                />
                            </ScrollArea>
                        </TabsContent>
                    </Tabs>
