...
function ListVoucher() {
    const { data: vouchers, isLoading } = useFetch<ApiResponse<PageResult<Voucher>>>(`/Vouchers`, [VOUCHER_KEY.VOUCHER_MANAGE])
                ...
                <div className="flex gap-2">
                    <Link href={
                        user?.value?.role === 'Administrator'
                            ? `/admin/voucher/${row.original.id}` :
                            `/manager/voucher/${row.original.id}`}>
                    ...
                    <Link href={
                        user?.value?.role === 'Administrator'
                            ? `/admin/voucher/update/${row.original.id}`
                            : `/manager/voucher/update/${row.original.id}`}>
    ...
    return (
        <div className='m-10'>
            ...
            <DeleteDialog
                ...
                refreshKey={[[VOUCHER_KEY.VOUCHER_MANAGE]]} />
        </div>
    )
}
