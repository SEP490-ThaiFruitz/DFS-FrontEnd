...
const ProductDetail = () => {
    const { id } = useParams();
    const { data: product } = useFetch<ApiResponse<Product>>(`/Products/manage/${id}`, [PRODUCT_KEY.PRODUCT_DETAIL_MANAGE, id as string])
            ...
            <div className='Container'>
                <Tabs defaultValue="information">
                    <TabsList className='grid grid-cols-5 gap-10 bg-transparent'>
                        ...
                        <TabsTrigger className='bg-white shadow-sm border data-[state=active]:!bg-sky-600 data-[state=active]:text-white' value="image">
                            <Images className='sm:mr-2' size={15} />
                            <span className="hidden sm:inline">{"Ảnh phụ"}</span>
                        </TabsTrigger>
                        <TabsTrigger className='bg-white shadow-sm border data-[state=active]:!bg-sky-600 data-[state=active]:text-white' value="nutrition">
                            <UtensilsCrossed className='sm:mr-2' size={15} />
                            <span className="hidden sm:inline">{"Dinh dưỡng"}</span>
                        </TabsTrigger>
                        <TabsTrigger className='bg-white shadow-sm border data-[state=active]:!bg-sky-600 data-[state=active]:text-white' value="certification">
