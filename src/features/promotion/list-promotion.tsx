...
const ListPromotion = () => {
    const { data: promotions, isLoading } = useFetch<ApiResponse<Promotion[]>>("/Promotions/manage", [PROMOTION_KEY.PROMOTION_MANAGE])
                    ...
                    <div className="flex items-center gap-2">
                        <Link href={
                            user?.value?.role === 'Administrator'
                                ? `/admin/promotion/${row.original.id}`
                                : `/manager/promotion/${row.original.id}`}>
                            <Button
                                variant="outline"
    ...
    return (
        <div className="m-10">
            ...
            <DeleteDialog
                ...
                refreshKey={[[PROMOTION_KEY.PROMOTION_MANAGE]]}
            />
        </div>
    )
}
