...
const PointTab = () => {
    const { isPending, data: pointsHistory } = useFetch<ApiResponse<PageResult<Point>>>("/PointHistories", ["PointHistories"])
    ...
    return (
        isPending ?
            Array.from({ length: 3 }).map((_, index) => (
                <div
                    key={index + 1}
                    className="p-4 grid grid-cols-3 items-center gap-6 bg-white"
                >
            ...
            <div className="space-y-2">
                ...
                {pointsHistory?.value?.items.map((point: Point) => (
                    ...
                    >
                        ...
                        <p className={`text-xl font-bold text-center ${point.point > 0 ? 'text-green-600' : 'text-red-600'}`}>
                            {point.point > 0 ? `+${formatNumberWithUnit(point.point, "điểm")}` : `${formatNumberWithUnit(point.point, "điểm")}`}
                        </p>
