...
import { Bell, Boxes, Heart, LogOut, MapPinHouse, Menu as MenuIcon, UserRoundPen } from "lucide-react";
    ...
    <div className={cn(
      ...
      <Menu setActive={setActive} className="h-full flex items-center justify-between w-full rounded-none px-4">
        ...
        <div className="hidden md:flex items-center space-x-2">
          <Link href="/" className={navItemClassName}>Trang chủ</Link>
          <Link href="/find" className={navItemClassName}>Sản phẩm</Link>
          <MenuItem setActive={setActive} active={active} item="Quà tặng" className={navItemClassName}>
          ...
          <MenuItem setActive={setActive} active={active} item="Chính sách" className={navItemClassName}>
            <div className="flex flex-col space-y-4 text-sm">
              <HoveredLink href="/hobby">Hobby</HoveredLink>
              <HoveredLink href="/individual">Individual</HoveredLink>
              <HoveredLink href="/team">Team</HoveredLink>
          ...
          <Link href="/blogs">
            <MenuItem setActive={setActive} active={active} item="Bài viết" className={navItemClassName}>
              <div className="flex flex-col space-y-4 text-sm">
        ...
        <div className="flex items-center space-x-2">
          {user ? (
            <>
              ...
              <Popover>
                <PopoverTrigger>
                  <div className="relative inline-flex text-sm h-11 w-10 items-center justify-center text-neutral-800 dark:text-neutral-300 hover:bg-neutral-500/20 rounded-[14px] cursor-pointer transition">
                    ...
                    <span className="absolute top-1 -right-1 w-4 h-4 bg-primary-500
                    text-slate-900 rounded-full flex items-center justify-center">0</span>
                  </div>
                </PopoverTrigger>
              ...
              <Link href="/favorites" className="h-11 w-10 flex items-center justify-center hover:bg-neutral-500/20 rounded-[14px]">
                <Heart className="size-4" />
              </Link>
              <ShoppingBagSheet />
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="h-12 w-12 rounded-full border">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 right-0 bg-neutral-100/95 dark:bg-neutral-800/95 border-t shadow-lg">
          <div className="flex flex-col p-4 space-y-4">
            <Link href="/" className={navItemClassName} onClick={() => setIsMobileMenuOpen(false)}>Trang chủ</Link>
            <Link href="/find" className={navItemClassName} onClick={() => setIsMobileMenuOpen(false)}>Sản phẩm</Link>
            <MenuItem setActive={setActive} active={active} item="Quà tặng" className={navItemClassName}>
