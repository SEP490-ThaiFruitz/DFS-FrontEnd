...
import { FEEDBACK_KEY } from "@/app/key/comm-key";
...
function ListFeedback() {
    const { data: feedbacks, isLoading } = useFetch<
        ApiResponse<FeedbackResponse>
    >("/Feedbacks", [FEEDBACK_KEY.FEEDBACK_MANAGE]);
    ...
    const filterFeedbacks = feedbacks?.value?.feedbackResponse.items.filter(
        (feedback) => {
    ...
    const columns: ColumnDef<Feedback>[] = [
        {
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-3">
                    {row.original.productFeedback.image !== null ? (
                        <Image
                            className="h-40 w-40"
                        ...
                        row.original.productFeedback.customImages !== null && (
                            <div className="size-20 relative rounded-lg overflow-hidden bg-slate-50 border border-slate-200 shadow-sm group mr-4">
                                ...
                                <div className="absolute inset-0 z-10">
                                    <Image
                                        src={
                                            row.original.productFeedback.customImages[0] ||
                                            "/placeholder.svg"
                                        }
                                ...
                                {row.original.productFeedback.customImages.length > 1 && (
                                    <div className="absolute right-1 bottom-1 z-20 flex flex-col-reverse gap-1">
                                        {row.original.productFeedback.customImages
                                            .slice(1, 3)
                                            .map((image, index) => (
                                                <div
                                                    key={index}
                    ...
                    <div className="mt-3">
                        <div className="font-medium">
                            {row.original.productFeedback.name}
                        </div>
                        {row.original.type !== "Combo" && (
                            <div className="flex items-center gap-3">
                                <div className="text-sm text-muted-foreground">
                                    {row.original.productFeedback.packageType}
                                </div>
                                <span>•</span>
                                <div className="text-sm text-muted-foreground">
                                    {row.original.productFeedback.netWeight} g
                                </div>
                            </div>
                        )}
                    </div>
            ...
            cell: ({ row }) => (
                <div className="flex items-center">
                    {Array.from({ length: 5 }).map((_, index) => (
                        <Star
                            key={index}
                            className={`w-4 h-4 ${index < row.original.stars
        ...
        },
        {
            ...
            cell: ({ row }) =>
                row.original.isShow ? (
                    <div className="bg-green-50 text-green-600 font-semibold w-fit py-1 px-2 rounded-lg">
                        Đang hiện
                    </div>
                ) : (
                    <div className="bg-red-50 text-red-600 font-semibold w-fit py-1 px-2 rounded-lg">
                        Đã ẩn
                    </div>
                ),
            enableSorting: true,
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div>
                    {formatTimeVietNam(new Date(row.original.createdOnUtc), true)}
                </div>
            ),
            enableSorting: true,
            enableColumnFilter: true,
    ...
    const hideShowFeedback = async (id: string) => {
        return await API.patch(`/Feedbacks/${id}/status`, {});
    }
    ...
    return (
        <div className="m-10">
                ...
                <>
                    <div className="grid gap-8 md:gap-16 md:grid-cols-2 lg:grid-cols-4 mt-10">
                        <Card className="transition-all hover:shadow-md cardStyle">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">
                                    {feedbacks?.value?.feedbackStatistic.totalFeedback ?? 0}
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="transition-all hover:shadow-md cardStyle">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">
                                    {feedbacks?.value?.feedbackStatistic.average ?? 0}
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="transition-all hover:shadow-md cardStyle">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">
                                    {feedbacks?.value?.feedbackStatistic.positive ?? 0}
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="transition-all hover:shadow-md cardStyle">
                            ...
                            <CardContent>
                                <div className="text-2xl font-bold">
                                    {feedbacks?.value?.feedbackStatistic.hide ?? 0}
                                </div>
                            </CardContent>
                        </Card>
                    </div>
                            ...
                            <div className="flex items-center justify-end pr-4">
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button
                                            variant="outline"
                                            className="min-w-fit justify-between sm:w-auto"
                                        >
                                    ...
                                    <DropdownMenuContent align="start">
                                        <DropdownMenuItem onClick={() => setSeletedOption("All")}>
                                            Tất cả
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                                            onClick={() => setSeletedOption("Active")}
                                        >
                                            Đang hiện
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                                            onClick={() => setSeletedOption("IsDeleted")}
                                        >
                                            Đã ẩn
                                        </DropdownMenuItem>
                                    </DropdownMenuContent>
            ...
            <DeleteDialog
                ...
                content={`Bạn có chắc chắn muốn ${feedback?.isShow ? "ẩn" : "hiện"
                    } đánh giá này không?`}
                deleteFunction={hideShowFeedback}
                refreshKey={[[FEEDBACK_KEY.FEEDBACK_MANAGE]]}
            />
        </div>
    );
}
