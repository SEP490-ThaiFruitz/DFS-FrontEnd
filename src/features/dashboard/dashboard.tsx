...
const Dashboard = () => {
    ...
    const { data: reportData, refetch, isLoading } = useFetch<ApiResponse<RevenueReport>>(
        formattedUrl,
        [PRODUCT_KEY.PRODUCT_VARIANT_STATISTIC_MANAGE]
    );
    ...
    const { data: reportDataOld, refetch: refreshOld } = useFetch<ApiResponse<RevenueReport>>(
        formattedOldUrl,
        [PRODUCT_KEY.PRODUCT_VARIANT_OLD_STATISTIC_MANAGE]
    );
        ...
        },
        {
            ...
            cell: ({ row }) =>
                <Link href={user?.value?.role === 'Administrator'
                    ? `/admin/dashboard/${row.original.productId}`
                    : `/manager/dashboard/${row.original.productId}`}>
                    <Button
                        variant="outline"
